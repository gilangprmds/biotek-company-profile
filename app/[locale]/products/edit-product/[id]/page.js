...
export default function EditProduct() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch categories
        const catResponse = await fetch('https://biotek.co.id/api/product-category');
        const catData = await catResponse.json();
        setCategories(catData.data);

        // Fetch product data
        const prodResponse = await fetch(`https://biotek.co.id/api/product/${id}`);
  ...
  const openImageEditor = async (index) => {
    ...
    try {
      ...
      if (image.type === 'existing') {
        ...
        const response = await fetch(`https://biotek.co.id/api${image.url}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`https://biotek.co.id/api/product/update/${id}`, {
        method: 'PUT',
        body: formData,
      });
                          ...
                          <div className="relative aspect-square">
                            <img 
                              src={preview.type === 'existing' 
                                ? `https://biotek.co.id/api${preview.url}` 
                                : preview.url}
                              alt={`Preview ${index}`} 
