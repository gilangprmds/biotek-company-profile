...
const openSubCategoryModal = (subCategory = null) => {
  ...
  const fetchCategories = async () => {
    try {
      setIsLoading(true);
      const res = await fetch('http://localhost:8080/product-category');
  ...
  const calculateCategoryCounts = async () => {
    try {
      ...
      const res = await fetch('http://localhost:8080/product?size=1000');
      const data = await res.json();
                                ...
                                <div className="flex items-center">
                                  {product.productImages?.[0]?.urlImage && (
                                    <div className="flex-shrink-0 h-20 w-20">
                                      <img className="h-20 w-20 rounded-md object-cover" src={`http://localhost:8080${product.productImages?.[0]?.urlImage}`} alt={product.name} />
                                    </div>
                                  )}
                                  <div className="ml-4">
                                  ...
                                  <div className="mt-4">
                                    ...
                                    <div className="flex gap-2 overflow-x-auto pb-2">
                                      {product.productImages?.map((img, idx) => (
                                        <img 
                                          key={idx} 
                                          src={`http://localhost:8080${img.urlImage}`} 
                                          alt={`Produk ${idx+1}`} 
