...
//   { key: 'noProduct', label: 'Product Code' },
//   { key: 'modelOrType', label: 'Model / Type' },
//   { key: 'unitsOfMeasurement', label: 'Unit of Measurement' },
//   { key: 'kbkiCode', label: 'KBKI Code' },
//   { key: 'permitToCirculateNumber', label: 'Permit Number' },
//   { key: 'factoryName', label: 'Factory Name' },
//   { key: 'productValidityPeriod', label: 'Validity Period' },
// ];

// function SortableImage({ src, index, onRemove, id }) {
//   const {
//     attributes,
//     listeners,
//     setNodeRef,
...
//       <img src={src} alt={`Preview ${index}`} className="rounded-md h-24 w-full object-cover" />
//       <div className="absolute top-1 left-1 bg-white p-1 rounded-full shadow">
...
//     productFields.reduce((acc, field) => ({ ...acc, [field.key]: '' }), {})
//   );
//   const [images, setImages] = useState([]);
//   const [previewImages, setPreviewImages] = useState([]);
...
//     // Pastikan ukuran persegi (pakai sisi terpendek)
...
//         setPreviewImages((prev) => [...prev, URL.createObjectURL(blob)]);
//       }
//     }, 'image/jpeg');
...
//       alert('Some files were too large (max 2MB) and were skipped.');
//     }
//     if (validFiles.length > 0) {
//       showCropper(validFiles[0]);
//     }
//   };
...
//       const oldIndex = previewImages.findIndex((_, idx) => idx.toString() === active.id);
//       const newIndex = previewImages.findIndex((_, idx) => idx.toString() === over.id);
//       setPreviewImages((items) => arrayMove(items, oldIndex, newIndex));
...
//     try {
//       const res = await fetch('http://localhost:8080/api/products/save', {
//         method: 'POST',
//         body: formData,
//       });
...
//         className="grid md:grid-cols-3 gap-8 bg-white p-8 rounded-xl shadow-lg"
//       >
//         <div className="md:col-span-2 space-y-4">
//           {productFields.map(({ key, label, required }) => (
//             <div key={key}>
...
//             >
//               <PlusIcon size={32} />
//               <span className="text-sm mt-1">Click or drag files here</span>
//             </label>
//           </div>
...
//               <SortableContext
//                 items={previewImages.map((_, idx) => idx.toString())}
